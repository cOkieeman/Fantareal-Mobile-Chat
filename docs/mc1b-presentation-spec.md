...
| --- | --- | --- | --- |
| `closed` | 无 | 销毁前台 Web/service session；停止页面内工作 | 无运行页面 |
| `compact` | 默认 `500×860`；最小 `480×760`；最大 `600×980` | 非模态独立窗口，允许与主 Chat 并行 | 单内容列；主页和群聊纵向切换 |
| `expanded` | 默认 `1024×860`；最小 `760×700`；最大 `1280×1000` | 非模态独立窗口，沿用同一 session | 左侧应用导航 + 右侧内容；不复制第二套业务页面 |

`390×700` 继续作为窄屏异常恢复和浏览器回归尺寸，不再作为 100% DPI 下的日常默认窗口。
...
Host 应遵守 expanded 的 `760px` 最小逻辑宽度。窄于该值的规则只用于工具容器、浏览器调试和异常恢复，不是日常 expanded 窗口目标。
...
- 关闭结束前台 session；页面关闭后不得继续运行定时器、模型请求或 service。
...
- `390×700` 窄屏恢复、`500×860` compact、`1024×860` expanded 与 `1180×820` 大屏回归均无裁切或不可达操作。
